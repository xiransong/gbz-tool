...
void GBZTool::find_node_info(const std::string& gbz_file, gbwt::node_type node_id) {
    try {
        gbwtgraph::GBZ local_gbz;
        std::ifstream in(gbz_file, std::ios::binary);
        if (!in.is_open()) {
            std::cerr << "Error: Could not open GBZ file: " + gbz_file;
            return;
        }
        ...
        if (!graph.has_node(node_id)) {
            std::cerr <<  "Error: Node " + std::to_string(node_id) + " does not exist.";
            return;
        }

        // get the string that contains node info in json format
        std::string s_node_info = this->_get_node_info(local_gbz.index, local_gbz.graph, node_id);
        std::cout << s_node_info << std::endl;

    } catch (const std::exception& e) {
        std::cerr <<  "Error processing GBZ file: " + std::string(e.what());
        return;
    }
}
...
void GBZTool::find_batch(const std::string& gbz_file, const std::string& node_ids_file, const std::string& output_json) {
    ...
    // Query node information using _get_node_info
    json results;
    const gbwtgraph::GBWTGraph& graph = gbz.graph;
    for (gbwt::node_type node_id : node_ids) {
        if (!graph.has_node(node_id)) {
            results[std::to_string(node_id)] = {
                {"error", "Node " + std::to_string(node_id) + " does not exist."}
            };
            continue;
        }

        try {
            // Get node info as JSON string
            std::string s_node_info = this->_get_node_info(gbz.index, gbz.graph, node_id);
            // Parse the JSON string, removing the "node_id: " prefix
            size_t colon_pos = s_node_info.find(": ");
            if (colon_pos != std::string::npos) {
                std::string json_str = s_node_info.substr(colon_pos + 2);
                results[std::to_string(node_id)] = json::parse(json_str);
            } else {
                results[std::to_string(node_id)] = {
                    {"error", "Failed to parse node info for node " + std::to_string(node_id)}
                };
            }
        } catch (const std::exception& e) {
            results[std::to_string(node_id)] = {
                {"error", "Error processing node " + std::to_string(node_id) + ": " + e.what()}
            };
        }
    }
    ...
    out_json << results.dump(2) << std::endl;
    out_json.close();
